...
public ClientConVar1(QueryCookie cookie, int client, ConVarQueryResult result, const char[] cvarName, const char[] cvarValue)
{
    ...
    else if (StringToFloat(cvarValue) > 0.100000)
    {
        ...
        CPrintToChatAll("{hotpink}[NetLimiter]{white} Player %N was using CVar %s = %s, kicked from server.", client, cvarName, cvarValue);
    }
}
...
public ClientConVar2(QueryCookie cookie, int client, ConVarQueryResult result, const char[] cvarName, const char[] cvarValue)
{
    ...
    else if ((StringToFloat(cvarValue) < 60) || SimpleRegexMatch(cvarValue, "^[0-9]*$") <= 0)
    {
        ...
        LogMessage("[NetLimiter] Player %N is using CVar %s = %s, likely ping-masking!", client, cvarName, cvarValue);
        //CPrintToChatAll("{hotpink}[NetLimiter]{white} Player %N is using CVar %s = %s, likely ping-masking! cl_cmdrate should be at or above 60 to prevent this.", client, cvarName, cvarValue);
    }
}
